...
archive_path="release/conductor-client-macos.tar.gz"
...
usage() {
  cat >&2 <<'EOF'
Usage: validate-macos-client.sh [options]

Options:
  --archive-path <path>      macOS client archive to validate.
...
append_command_summary "xcodebuild" xcodebuild -version

exec > >(tee "$log_path") 2>&1

echo "macOS client flow smoke"
...
if [[ "$skip_client_build" -eq 0 ]]; then
  echo ""
  echo "==> Build macOS client package"
...
if [[ ! -f "$archive_path" ]]; then
  echo "macOS client archive not found: $archive_path. Run without --skip-client-build or build the package first." >&2
  exit 1
fi
...
./scripts/verify-client-archive.sh macos "$archive_path"

echo ""
echo "==> Smoke launch macOS client"
./scripts/smoke-client-launch.sh macos "$archive_path"

echo ""
echo "==> Smoke register through macOS client"
CONDUCTOR_CLIENT_E2E_EVIDENCE_DIR="$e2e_log_dir" ./scripts/smoke-macos-client-e2e.sh "$archive_path"

echo ""
echo "macOS client flow smoke passed: $archive_path"
write_result "passed"
...
./scripts/verify-macos-smoke-evidence.sh "${verify_args[@]}"

echo "macOS controlled client validation passed."
