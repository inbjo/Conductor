...
if [[ ! -f "$e2e_server_log" ]]; then
  echo "Missing macOS client e2e server log: $e2e_server_log" >&2
  exit 1
fi
if [[ ! -f "$e2e_client_log" ]]; then
  echo "Missing macOS client e2e client log: $e2e_client_log" >&2
  exit 1
fi
if [[ ! -f "$e2e_settings_file" ]]; then
  echo "Missing macOS client e2e settings file: $e2e_settings_file" >&2
  exit 1
fi
...
if [[ ! -f "$evidence_archive_sidecar" ]]; then
  echo "macOS smoke evidence archive checksum sidecar is missing: $evidence_archive_sidecar" >&2
  exit 1
fi
evidence_sidecar_sha256="$(awk '{print $1; exit}' "$evidence_archive_sidecar")"
if [[ "$evidence_sidecar_sha256" != "$archive_sha256" ]]; then
  echo "macOS smoke evidence archive sidecar hash mismatch. Summary: $archive_sha256 Sidecar: $evidence_sidecar_sha256" >&2
  exit 1
fi
if [[ -f "$archive_path" ]]; then
  ...
  if [[ ! -f "$archive_path.sha256" ]]; then
    echo "macOS smoke evidence archive checksum sidecar is missing: $archive_path.sha256" >&2
    exit 1
  fi
  sidecar_sha256="$(awk '{print $1; exit}' "$archive_path.sha256")"
  if [[ "$sidecar_sha256" != "$archive_sha256" ]]; then
    echo "macOS smoke archive sidecar hash mismatch. Summary: $archive_sha256 Sidecar: $sidecar_sha256" >&2
    exit 1
  fi
else
...
if ! grep -q "agent config .*server_url=ws://127\.0\.0\.1:.*/ws/agent" "$e2e_client_log"; then
  echo "macOS client e2e client log does not prove normalized serverUrl propagation." >&2
  exit 1
fi
...
if ! grep -q '"serverUrl": "ws://127\.0\.0\.1:.*\/ws\/agent"' "$e2e_settings_file"; then
  echo "macOS client e2e settings file does not contain the normalized serverUrl." >&2
  exit 1
fi
...
if ! grep -q '"interactiveApproval": false' "$e2e_settings_file"; then
  echo "macOS client e2e settings file does not contain interactiveApproval=false." >&2
  exit 1
fi
