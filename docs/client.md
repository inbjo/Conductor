...
脚本会写出 `validation-summary.txt`、`smoke-windows-client-flow.log` 和 `logs/` 下的原始 e2e 日志及 `client-settings.json`，用于记录 runner/工具链版本、完整 smoke 输出、Settings 持久化结果和 server/client/agent 运行细节。
...
`validate-macos-client.sh` 会构建客户端包、构建 Web 静态资源和 smoke server、校验 tar.gz、启动 `.app` GUI 入口 smoke，再通过 `CONDUCTOR_CLIENT_AUTOSTART=1` 启动 `.app` 内的 Flutter 客户端，验证客户端能自动拉起包内 `conductor-agent`、传递运行配置并注册到本地 smoke server。脚本会写出 `artifacts/macos-client-smoke/validation-summary.txt`、`smoke-macos-client-flow.log`、归档 `.sha256` sidecar、`logs/client-e2e/` 原始 server/client 日志和 `client-settings.json`，CI 会上传为 `macos-client-smoke-evidence`。
