...
| macOS | `release/conductor-client-macos.tar.gz` | 自动化覆盖 | 自动化覆盖 | 自动化覆盖 | 自动化覆盖 | 待真机验证 |
...
- 最近验证：2026-06-28，commit `6e8347c`
- 最近归档 SHA256：`99b96b599fcf0fd08893b5d28c6e7c03340c95fda6a9e05226eca40c73823e13`
...
`validate-linux-client.sh` 会写出 `artifacts/linux-client-smoke/validation-summary.txt`、`smoke-linux-client-flow.log`、归档 `.sha256` sidecar 和 `logs/client-e2e/` 原始 server/client 日志及 `client-settings.json`。CI 中的 `client-linux` job 会上传 `linux-client-smoke-evidence` artifact。
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\validate-windows-client.ps1 -SkipClientBuild -SkipServerBuild
```
...
- `Validate Windows client flow` 步骤成功。
...
- `windows-client-smoke-evidence/smoke-windows-client-flow.log` 出现 `Agent config log observed`，证明 Flutter 客户端把 Server URL、Token、Agent Name、File Root、Audio Input 和本地审批配置传给了包内 Agent。
- `windows-client-smoke-evidence/smoke-windows-client-flow.log` 出现 `Agent diagnostics observed`，证明客户端可向 Agent stdin 发送本地诊断命令。
- `windows-client-smoke-evidence/logs/agent-e2e/` 和 `windows-client-smoke-evidence/logs/client-e2e/` 存在原始 server/client/agent 日志及 `client-settings.json`，且 agent/client 日志包含 `agent config`，Settings JSON 包含规范化后的 `serverUrl`、`agentToken`、`agentName`、`agentRoot`、`audioInput` 和 `interactiveApproval=false`。
- `windows-client-smoke-evidence/` 会保留归档 `.sha256` sidecar；`Verify Windows smoke evidence` 会检查 sidecar 与 `archive_sha256` 一致。如果归档仍在工作区内，也会复算归档 SHA256。

所有 Windows smoke evidence 都必须记录 `commit`。CI 会通过 `-RequireCiFields -ExpectedCommit $env:GITHUB_SHA` 额外强制检查 `runner_os` 和 `runner_arch`，并确认 evidence 的 commit 与当前 workflow commit 一致；手工真机验收会用 `git rev-parse HEAD` 记录 `commit`，但默认不要求 runner OS/arch 这些 CI 专属字段。
...
如果已经构建过包和 `target/debug/conductor-server`，可只重复校验和 smoke：

```sh
./scripts/validate-macos-client.sh --skip-client-build --skip-server-build
```

需要保留验收证据时，可以指定输出目录：

```sh
./scripts/validate-macos-client.sh --evidence-dir artifacts/macos-client-smoke
```

脚本会写出 `validation-summary.txt`、`smoke-macos-client-flow.log`、归档 `.sha256` sidecar 和 `logs/client-e2e/` 原始 server/client 日志及 `client-settings.json`，用于记录 commit、runner/工具链版本、归档 SHA256、完整 smoke 输出和 e2e 运行细节。
CI 中的 `client-macos` job 会上传 `macos-client-smoke-evidence` artifact。判断 macOS 自动化是否真正通过时，需要同时确认：

- `Validate macOS client flow` 步骤成功。
- `macos-client-smoke-evidence/validation-summary.txt` 记录了 commit、runner、Rust、Flutter、Xcode 和 `archive_sha256=<sha256>`。
- `macos-client-smoke-evidence/validation-summary.txt` 记录 `result=passed`。
- `macos-client-smoke-evidence/smoke-macos-client-flow.log` 末尾出现 `macOS client flow smoke passed`。
- `macos-client-smoke-evidence/smoke-macos-client-flow.log` 出现 `Agent config log observed`，证明 Flutter 客户端把运行配置传给了包内 Agent。
- `macos-client-smoke-evidence/smoke-macos-client-flow.log` 出现 `Agent diagnostics observed`，证明客户端可向 Agent stdin 发送本地诊断命令。
- `macos-client-smoke-evidence/logs/client-e2e/server.log`、`client.log` 和 `client-settings.json` 存在，且 client 日志包含 `agent config` 和 `[diagnostics] conductor-agent`，Settings JSON 包含规范化后的 `serverUrl`、`agentToken`、`agentName`、`agentRoot`、`audioInput` 和 `interactiveApproval=false`。
- `macos-client-smoke-evidence/` 会保留归档 `.sha256` sidecar；`Verify macOS smoke evidence` 会检查 sidecar 与 `archive_sha256` 一致。如果归档仍在工作区内，也会复算归档 SHA256。
...
| Job | Runner | 覆盖 |
| --- | --- | --- |
| `static-checks` | `ubuntu-24.04` | GitHub/Gitea workflow 镜像检查、shell 脚本语法检查、客户端 helper usage 检查、aggregate/platform smoke evidence 合成测试、客户端构建默认参数传递测试、macOS metadata 检查。 |
| `powershell-static-checks` | `windows-2022` | Windows PowerShell 脚本语法解析检查，并运行 Windows 归档校验、smoke evidence 和构建默认参数合成测试。 |
...
| `client-smoke-evidence` | `ubuntu-24.04` | 下载 Linux/Windows/macOS smoke evidence artifact，统一校验 commit、runner 字段、归档 SHA、`.sha256` sidecar、成功标记和原始 e2e 日志。 |
