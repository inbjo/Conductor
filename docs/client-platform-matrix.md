...
- `windows-client-smoke-evidence/logs/agent-e2e/` 和 `windows-client-smoke-evidence/logs/client-e2e/` 存在原始 server/client/agent 日志及 `client-settings.json`，且独立 Agent e2e 和客户端 e2e 日志都包含规范化后的 `server_url=ws://127.0.0.1:<port>/ws/agent`，Settings JSON 包含规范化后的 `serverUrl`、`agentToken`、`agentName`、`agentRoot`、`audioInput` 和 `interactiveApproval=false`。
