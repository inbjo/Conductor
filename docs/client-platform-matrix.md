...
CI 会通过 `-RequireCiFields -ExpectedCommit $env:GITHUB_SHA` 强制检查 `commit`、`runner_os` 和 `runner_arch`，并确认 evidence 的 commit 与当前 workflow commit 一致；手工真机验收默认不要求这些 CI 专属字段。
...
脚本会写出 `validation-summary.txt` 和 `smoke-macos-client-flow.log`，用于记录 runner/工具链版本、归档 SHA256 和完整 smoke 输出。
