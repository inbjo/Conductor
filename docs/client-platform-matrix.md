...
- `windows-client-smoke-evidence/logs/agent-e2e/` 和 `windows-client-smoke-evidence/logs/client-e2e/` 存在原始 server/client/agent 日志及 `client-settings.json`，且 agent/client 日志包含 `agent config`，Settings JSON 包含规范化后的 `serverUrl`。
...
- `macos-client-smoke-evidence/logs/client-e2e/server.log`、`client.log` 和 `client-settings.json` 存在，且 client 日志包含 `agent config` 和 `[diagnostics] conductor-agent`，Settings JSON 包含规范化后的 `serverUrl`。
