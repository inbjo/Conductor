...
`validate-linux-client.sh` 会写出 `artifacts/linux-client-smoke/validation-summary.txt`、`smoke-linux-client-flow.log` 和 `logs/client-e2e/` 原始 server/client 日志。CI 中的 `client-linux` job 会上传 `linux-client-smoke-evidence` artifact。
...
- `windows-client-smoke-evidence/logs/agent-e2e/` 和 `windows-client-smoke-evidence/logs/client-e2e/` 存在原始 server/client/agent 日志，且 agent/client 日志包含 `agent config`。
...
脚本会写出 `validation-summary.txt`、`smoke-macos-client-flow.log` 和 `logs/client-e2e/` 原始 server/client 日志，用于记录 commit、runner/工具链版本、归档 SHA256、完整 smoke 输出和 e2e 运行细节。
...
- `macos-client-smoke-evidence/logs/client-e2e/server.log` 和 `macos-client-smoke-evidence/logs/client-e2e/client.log` 存在，且 client 日志包含 `agent config` 和 `[diagnostics] conductor-agent`。
