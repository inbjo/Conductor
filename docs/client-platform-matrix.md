...
CI 会通过 `-RequireCiFields -ExpectedCommit $env:GITHUB_SHA` 强制检查 `commit`、`runner_os` 和 `runner_arch`，并确认 evidence 的 commit 与当前 workflow commit 一致；手工真机验收会用 `git rev-parse HEAD` 记录 `commit`，但默认不要求 runner OS/arch 这些 CI 专属字段。
