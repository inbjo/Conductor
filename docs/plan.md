...
- 构建 `web/dist`。
- 构建内嵌 Web 的 Server 二进制。
- 构建 Windows、macOS、Linux Agent。
- 编写运行说明和演示脚本。
- 打包源码、程序和文档。

验收标准：

- 运行 Server 二进制即可打开 Web 后台。
- Agent 启动后能出现在设备列表。
- 可以完成远控、文件、文字沟通演示。
- 提交包包含源码和文档。
...
1. 启动 Rust Server。
2. 浏览器打开 Web 后台并登录。
3. 在 Windows/macOS/Linux 任一设备启动 Agent。
...
- 跨平台 Agent 难度高：优先保证当前开发平台完整可演示，代码结构预留 Windows/macOS/Linux 适配。
...
- 一个 Rust Server 二进制即可提供后台页面和 API。
- 一个 Agent 程序即可接入后台并显示在线。
- 管理员通过浏览器完成远程控制演示。
- 管理员可以进行基本文件操作。
- 管理员和被控端用户可以进行文字沟通。
- 文档清晰说明架构、运行方式、演示流程和后续扩展方向。
