...
- macOS 客户端：安装 Rust/Flutter，执行 `scripts/build-client.sh`，校验归档，做 `.app` 启动 smoke，并通过 e2e smoke 验证 Flutter 客户端可自动拉起包内 Agent 注册到本地 smoke server。
...
三端 smoke evidence 除 `validation-summary.txt` 和 `smoke-*-client-flow.log` 外，还会保留 e2e 原始日志：Linux/macOS 在 `logs/client-e2e/`，Windows 在 `logs/agent-e2e/` 和 `logs/client-e2e/`。证据校验会要求这些日志存在，并检查 Agent 配置传递日志。
