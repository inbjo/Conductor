...
| `powershell-static-checks` | `windows-2022` | 不产出包；检查 Windows PowerShell 脚本语法，并运行 Windows client archive checksum synthetic 测试。 |
...
- PowerShell 静态检查：在 Windows runner 上解析 `scripts/*.ps1`，提前发现语法错误，并运行 Windows client archive checksum synthetic 测试。
