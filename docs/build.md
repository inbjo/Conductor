...
- `Audio Input`：可留空，默认使用 Agent 内置音频输入策略
- `Require local approval`：是否要求本地审批远控请求
...
| Job | Runner | 产物 |
| --- | --- | --- |
| `static-checks` | `ubuntu-24.04` | 不产出包；检查 workflow 镜像、shell 脚本语法、客户端 helper usage、synthetic evidence 聚合摘要、平台 smoke evidence verifier、synthetic archive checksum、客户端构建默认参数传递和 macOS metadata。 |
| `powershell-static-checks` | `windows-2022` | 不产出包；检查 Windows PowerShell 脚本语法，并运行 Windows client archive checksum、smoke evidence 和构建默认参数 synthetic 测试。 |
...
| `client-smoke-evidence` | `ubuntu-24.04` | `client-smoke-evidence-verified`，包含三端已统一复核的 smoke evidence 和聚合摘要。 |
...
- 静态检查：确认 GitHub/Gitea workflow 内容一致、shell 脚本可解析、客户端 helper usage 入口可执行，运行 synthetic evidence 聚合摘要测试、平台 smoke evidence verifier 测试、synthetic archive checksum 测试、客户端构建默认参数传递测试，并检查 macOS `Info.plist`/entitlements 的麦克风、网络和沙箱配置。
- PowerShell 静态检查：在 Windows runner 上解析 `scripts/*.ps1`，提前发现语法错误，并运行 Windows client archive checksum、smoke evidence 和构建默认参数 synthetic 测试。
...
三端 smoke evidence 除 `validation-summary.txt` 和 `smoke-*-client-flow.log` 外，还会保留归档 `.sha256` sidecar、e2e 原始日志和 `client-settings.json`：Linux/macOS 在 `logs/client-e2e/`，Windows 在 `logs/agent-e2e/` 和 `logs/client-e2e/`。证据校验会要求这些文件存在，并检查 summary 中的归档 SHA256、evidence 内的 `.sha256` sidecar、原始归档 sidecar、HTTP Server URL 输入记录、Agent 配置传递日志、`/diagnostics` 输出，以及 Settings 中规范化后的 `serverUrl`、`agentToken`、`agentName`、`agentRoot`、`audioInput` 和 `interactiveApproval=false`。
...
- Server 是否启动。
- Settings 页中的 `Server URL` 是否指向正确 Server。客户端保存 Settings 时会自动把 `http://host:port` 或 `host:port` 转成 `ws://host:port/ws/agent`，非法 URL 会停留在 Settings 页并提示错误。
